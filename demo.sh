...
case "$1" in
  ...
  show)     exec $PY compare.py --task "${2:-movies_per_year}" ;;  # THE DEMO: cold vs warm
  fail)     exec $PY agent.py --task "${2:-movies_per_year}" --mode cold ;;
  pass)     exec $PY agent.py --task "${2:-movies_per_year}" --mode warm ;;
  ...
  *)
    ...
    echo "  show [task]      THE DEMO: cold vs warm side by side"
